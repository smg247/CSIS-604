...
public class Driver {
    ...
    public static void main(String[] args) {
        ...
        final int numberOfThreads = args[3] != null ? Integer.parseInt(args[3]) : 3;

        if (generateFile) {
            createRandomNumbersFile(amountOfNumbers);
        }

        List<List<Integer>> subLists = createSubLists(amountOfNumbers, numberOfThreads);

        List<Integer> mergeResult;
        if (usingThreads) {
            List<Thread> threads = sortByThreads(subLists);
            mergeResult = mergeResultsFromThreads(threads, subLists);
        } else {
            mergeResult = null;
        }
    ...
    private static List<Integer> mergeResultsFromThreads(List<Thread> threads, List<List<Integer>> subLists) {
